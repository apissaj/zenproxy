...
If Not IsUp("http://127.0.0.1:4096/api/health") Then
  ...
  WshShell.Run "cmd /c opencode serve --port 4096 --hostname 127.0.0.1 >> ""oc-serve.log"" 2>&1", 0, False
...
If Not IsUp("http://127.0.0.1:8020/health") Then
  WshShell.Run "cmd /c cd /d ""%~dp0"" && zenproxy.exe -port 8020 -config config.json >> zenproxy.log 2>&1", 0, False
End If